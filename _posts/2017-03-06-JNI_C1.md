...
- 高效性；为了支持对时间敏感的代码，本地方法接口必须占用很小的开销。为了确保虚拟机无关性（从而二进制兼容性），所有已知的技术都将会带来一定的开销。我们必、须在效率和虚拟机无关性中间达成某种妥协。
...
JNI并不是Java虚拟机所支持的唯一本地方法接口。那些喜欢将本地代码库加载到不同的Java虚拟机的开发者，将从标准接口获益。在某些情况下，开发者也许必须使用底层的虚拟机相关的接口，以达到高性能。在另一些情况下，开发者也许使用较高层的接口来创建软件模块。实际上，随着Java环境和软件构件技术的逐步成熟，本地代码将逐渐失去其意义。


----
<h5 id ='5'>使用JNI编程</h5>
本地方法开发者应该使用JNI编程。使用JNI编程可以将一些未知的内容隔离，例如终端用户使用的虚拟机。通过遵守JNI标准，本地库能够在给定虚拟机上很好得到运行。

如果你正在实现一个Java虚拟机，你应该实现JNI。JNI经过时间的验证，确保不会对你的虚拟机实现引入额外的开销和限制，包括对象的表示、垃圾回收框架等。当遇到任何我们忽视的问题时，请反馈给我们。


----
<h5 id ='6'>变化</h5>

对于Java SE 6.0，不提倡使用的结构体JDK1_1InitArgs和JDK1_1AttachArgs已经被移除了，取而代之的是JavaVMInitArgs和JavaVMAttachArgs。
...
[来源](http://docs.oracle.com/javase/8/docs/technotes/guides/jni/spec/intro.html)
